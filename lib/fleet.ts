...
async function fetchSnapshots(): Promise<Snapshot[]> {
  ...
  return results.filter(Boolean) as Snapshot[]
}
...
function buildFleetFromSnapshots(snapshots: Snapshot[]) {
  ...
  for (const [index, history] of map.entries()) {
    ...
    const altitudeRangeKm = altitudes.length ? Math.max(...altitudes) - Math.min(...altitudes) : 0
    const trendPoint = sortedHistory[Math.max(0, sortedHistory.length - 4)]
    const altitudeTrend3h =
      current.altitude != null && trendPoint?.altitude != null ? current.altitude - trendPoint.altitude : null
